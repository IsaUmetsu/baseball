    ...
    FROM
      (
        SELECT
          ...
          all_bat.team
        FROM
          baseball._situation_gyakuten_all AS all_bat -- 各打席の最終投球の情報を取得
          LEFT JOIN (
            SELECT
              date,
              location,
          ...
          AND all_bat.b_total = last_ball.b_total -- 最終投球 かつ 本塁打のみ抽出
        WHERE
    ...
    FROM
      ...
      (
        SELECT
          ...
          b_total
        FROM
          baseball._situation_gyakuten_all s -- 四球などは除外
