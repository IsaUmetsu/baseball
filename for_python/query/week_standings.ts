...
import { checkArgTMLGForTweet, checkArgTargetDay, displayResult, trimRateZero, checkLeague } from '../disp_util';
import { genTweetedDay, getIsTweet, tweetMulti, SC_WS, findSavedTweeted, saveTweeted, MSG_S, MSG_F } from '../tweet/tw_util';
...
(async () => {
  ...
  for (const teams of teamsArray) {
    ...
    if (getIsTweet()) {
      ...
      const savedTweeted = await findSavedTweeted(SC_WS, leagueArg, tweetedDay);
      const isFinished = await isFinishedGameByLeague(teams, tweetedDay);

      if (! savedTweeted && isFinished) {
        await tweetMulti(title, rows);
        await saveTweeted(SC_WS, leagueArg, tweetedDay);
        console.log(format(MSG_S, tweetedDay, leagueArg, SC_WS));
      } else {
        const cause = savedTweeted ? 'done tweet' : !isFinished ? 'not complete game' : 'other';
        console.log(format(MSG_F, tweetedDay, leagueArg, SC_WS, cause));
      }
    } else {
